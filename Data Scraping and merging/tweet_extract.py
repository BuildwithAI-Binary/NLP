'''
Script for extracting tweets

first create "auth.k" file each line containing given below values as follows:
api key
api secret keu
consumer key
consumer secret

FOR RUNNING THE SCRIPT USE AS FOLLOWS
python filename keyword -c no.of tweets

EX : python twitter_extract.py anxiety -c 1000000
'''
...
def tw_oauth(authfile):
    ...
    return tweepy.API(auth1,wait_on_rate_limit=True)
...
def tw_search_json(query, cnt=5):
    ...
    data = []
    for tweet in tweepy.Cursor(api.search, q=query,since="2019-10-01",until="2019-12-01",count=cnt).items():
        dTwt = {}
...
def tw_search(api):
    ...
    csvWriter.writerow(["created","Username", "text", "retwc", "hashtag",'location','source'])
    ...
    for tweet in tweepy.Cursor(api.search,
        ...
        try:
            temp = [i[u'text'] for i in tweet.entities[u'hashtags']]#hashtags used
            #print(temp)
            if len(temp) ==1:
		...
		# Dongho 03/28/16
        try:
            csvWriter.writerow([created,username, str(text).encode("utf-8"), retwc, hashtag,authorloc,source])
